const { toWei, expect, verifyRejection, zeroAddress, sqrt } = require("../utils");
...
describe('ScoreBySqrtCMC.sol', () => {
  ...
  function setupTests() {
    before(async () => {
      ...
      scoringStrategy = await deploy('ScoreBySqrtCMC', circulatingMarketCapOracle.address);
  ...
  describe('getTokenScores()', () => {
    ...
    it('Returns token market cap sqrts as scores', async () => {
      const tokens = [];
      const caps = [];
      for (let i = 0; i < 20; i++) {
      ...
      for (let i = 0; i < 20; i++) {
        expect(scores[i].eq(sqrt(caps[i]))).to.be.true;
      }
    })
  })
})